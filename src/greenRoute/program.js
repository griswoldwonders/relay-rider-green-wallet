...
export const PROGRAM = Object.freeze({
  ...
  monthlyKwhTenths: 500, // 50.0 kWh stored as tenths
...
export const PARTNER_SITES = Object.freeze([
  {
...
export function isWeekdayIsoDate(isoDate) {
...
export function remainingFromLedger(entries, monthlyCents = PROGRAM.monthlyBenefitCents, monthlyKwhTenths = PROGRAM.monthlyKwhTenths) {
  const consumedCents = entries
    .filter((row) => [LEDGER_STATUSES.redeemed, LEDGER_STATUSES.approved].includes(row.status))
    .reduce((sum, row) => sum + row.amountCents, 0);
  const consumedKwh = entries
    .filter((row) => [LEDGER_STATUSES.redeemed, LEDGER_STATUSES.approved, LEDGER_STATUSES.pending].includes(row.status))
...
export function defaultState() {
  ...
  return {
    ...
    claims: ledger.filter((row) => row.eventType.includes('claim') || row.eventType.includes('Claim')).map((row) => ({
      ...row,
      reviewNote: '',
    })),
    campaigns: [
