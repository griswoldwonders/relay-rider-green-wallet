...
import { loadProgramState, saveProgramState } from './storage.js';
import './greenRoute.css';
...
export default function GreenRouteCredits({ onOpenResearchLedger }) {
  ...
  function submitClaim() {
    ...
    persist({
      ...
      remainingKwhTenths: Math.max(0, state.remainingKwhTenths - checked.kwhTenths),
    });
    setClaimErrors([]);
    setClaimResult(estimate);
  }

  function reviewClaim(id, status, note) {
  ...
  return (
    <div className="grc-shell">
      ...
      <div className="grc-roles" role="group" aria-label="Prototype role">
        ...
        <button type="button" className="gw-secondary-button" onClick={onOpenResearchLedger}>Open research ledger</button>
      </div>
          ...
          <div className="grc-sites">
            {PARTNER_SITES.map((site) => (
              <article className="grc-site" key={site.id}>
                ...
                <div className="grc-actions">
                  <button type="button" className="gw-primary-button" onClick={() => setModal(site)}>Use voucher</button>
                </div>
              </article>
            ))}
          </div>
